...
function settings_page_html_form() { ?>
    <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
        <li class="nav-item" role="presentation">
            <button class="nav-link active" id="pills-request-log-tab" data-bs-toggle="pill" data-bs-target="#pills-request-log" type="button" role="tab" aria-controls="pills-request-log" aria-selected="false">
                <?= __('Request log', 'cf7lg'); ?>
            </button>
        </li>
        <li class="nav-item" role="presentation">
            <button class="nav-link" id="pills-apikey-tab" data-bs-toggle="pill" data-bs-target="#pills-apikey" type="button" role="tab" aria-controls="pills-apikey" aria-selected="true">
                <?= __('Settings cf7lg', 'cf7lg'); ?>
            </button>
        </li>
    </ul>
                    ...
                    <?php foreach($data as $item): ?>
                        <tr>
                            ...
                            <td class="d-none_" id="request">
                                <p><?= urldecode($item['request']); ?></p>
                                <input type="text" class="form-control d-none" id="server_response_<?= $item['id']; ?>" value="<?= htmlspecialchars($item['request']); ?>">
                            </td>
                            <td id="server_response"><?= $item['server_response']; ?></td>
                            <td>
                                <button type="button"
                                    <?php if($item['status_code'] == 200 || $item['status_code'] == 201) echo "disabled"; ?>
                                        onclick="repeatForm('server_response_<?= $item['id']; ?>', <?= $item['id']; ?>)"
...
function np_settings_apiKey_form() {
    ...
    echo '<input class="input_style" type="string" name="ifteam_apiKey" value="' . esc_attr($form) . '" />';
}
